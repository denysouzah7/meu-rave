...
import { getJoinedRoomSlugs, markRoomJoined } from "@/utils/joined-rooms";
...
export function DashboardPage() {
  ...
  const joinedRoomSlugs = React.useMemo(
    () => getJoinedRoomSlugs(me?.user.id),
    [me?.user.id],
  );
  ...
  React.useEffect(() => {
    ...
    for (const room of rooms) {
      if (room.hasJoined || room.creatorId === me.user.id) {
      ...
      <section className="grid gap-4 lg:grid-cols-[1fr_auto] lg:items-end">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
            Haru Space conecta grupos, raves, chat em tempo real, figurinhas,
            fotos, audios e enquetes.
          </p>
        </div>
        ...
        {isLoading ? (
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {[0, 1, 2].map((item) => (
              <div
                key={item}
                className="h-64 animate-pulse border border-white/10 bg-white/[0.05]"
              />
            ))}
          </div>
    ...
    <Card className="group flex h-full flex-col overflow-hidden rounded-none border-white/10 bg-white/[0.035] transition hover:border-primary/[0.40] hover:bg-white/[0.055] hover:shadow-glow">
      ...
      <CardContent className="flex min-h-[190px] flex-1 flex-col p-4">
        <div className="min-w-0">
          <h2 className="line-clamp-1 text-lg font-black leading-tight text-foreground">
            {room.name}
          </h2>
